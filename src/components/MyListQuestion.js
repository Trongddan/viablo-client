...
const MyListQuestion = ({ match }) => {
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(PostByUser(match.params.id,1));
  }, []);
  ...
  const clikDelete = (id) => {
    Swal.fire({
      ...
      text: "Bạn sẽ không thể khôi phục được thao tác này",
      icon: "warning",
    ...
    <>
      <div className="col-sm-11 mt-4 contentWrapper">
        <div className="row mb-3">
          <h6 className="col-sm-9">Câu hỏi của tôi</h6>
        </div>
        ...
        {posts? (
          posts.map((post, index) => (
              ...
              <div key={index} className="row">
                <div key={index} className="col-sm-11 user_Post_bg">
                  <Post
                    id={post.id}
                    key={post.id}
